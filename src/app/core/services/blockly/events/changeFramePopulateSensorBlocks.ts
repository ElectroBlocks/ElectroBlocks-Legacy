...
import { mapFakeSensorValuesToBlocks, FakeInputMap, SensorBlockMapper } from '../../player/frame/map_fake_sensor_values_to_blocks';
 ...
 */
export const changeFramePopulateSensorBlock = (changeFrame: FrameOutput, arduinoStartBlock: Block) => {
    ...
    mapToSetupSensorBlocks.forEach((mapToSetupSensorBlock) => {
        ...
        sensorBlocksToSetFakeValues
          .forEach((sensorBlockToSet) => setSensorBlockWithFakeValue(changeFrame, sensorBlockToSet, mapToSetupSensorBlock));
    });
...
const getListOfSensorBlockMappings = () => {
    ...
    const mapToSetupSensorBlocks = topBlocks
        ...
        .filter((block) =>Object.keys(mapFakeSensorValuesToBlocks).includes(block.type + '_block'))
        .map(setupBlock => mapFakeSensorValuesToBlocks[setupBlock.type + '_block']);

    // We manually add time no matter what
    mapToSetupSensorBlocks.push(
        mapFakeSensorValuesToBlocks['time_setup_block']
    );
 ...
 */
const isBlockInSetupArduinoFunction = (possibleBlock: Block, arduinoStartBlock: Block) => {
