 ...
 */
const math_round_block = (
  block: Block,
  frameLocation: FrameLocation,
  previousFrame?: ArduinoFrame
) => {
  const op = block.getFieldValue('OP');
  const numberString = getInputValue(
    block,
    'NUM',
    1,
    frameLocation,
 ...
 */
const math_modulo_block = (
  block: Block,
  frameLocation: FrameLocation,
  previousFrame?: ArduinoFrame
) => {
  const dividendValue = getInputValue(
    block,
    'DIVIDEND',
    1,
    frameLocation,
  ...
  const dividerValue = getInputValue(
    block,
    'DIVISOR',
    1,
    frameLocation,
  ...
  return parseInt(dividendValue, 0) % parseInt(dividerValue, 0);
};

/**
 * Gets a random number within range
 */
const math_random_int_block = (
  block: Block,
  frameLocation: FrameLocation,
  previousFrame?: ArduinoFrame
) => {
  const fromValue = getInputValue(
    block,
    'FROM',
    1,
    frameLocation,
  ...
  return getRandomInt(parseInt(fromValue, 0), parseInt(toValue, 0));
};

/**
 * Turns a "string" to a number not super relevant in javascript
 */
const string_to_number_block = (
  block: Block,
  frameLocation: FrameLocation,
  previousFrame?: ArduinoFrame
): number => {
  const numValue = getInputValue(
    block,
    'VALUE',
    0,
    frameLocation,
