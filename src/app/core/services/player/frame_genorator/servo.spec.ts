...
describe('servo block frame', () => {
  ...
  it('should not duplicate servos', () => {
    ...
    const [arduinoFrame] = rotate_servo_block(block1, {
      location: 'loop',
      iteration: 1
    });
    ...
    const [lastArduinoFrame] = rotate_servo_block(
      block2,
      { location: 'loop', iteration: 1 },
      arduinoFrame
    );
    ...
    const servoState = lastArduinoFrame.state.components
      ...
      .find(
        (servo: ServoState) => servo.pin === stringToPin('4')
      ) as ServoState;
  ...
  it('should generate 2 servos components if the pins are different', () => {
    ...
    const servoState2 = lastArduinoFrame.state.components
      .filter((servo) => servo instanceof ServoState)
      .find(
        (servo: ServoState) => servo.pin === stringToPin('6')
      ) as ServoState;
