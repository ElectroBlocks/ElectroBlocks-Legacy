...
describe('servo block frame', () => {
	...
	it ('should not duplicate servos', () => {
		...
		const [arduinoFrame] = rotate_servo_block(block1, { location: 'loop', iteration: 1 });
		...
		const [lastArduinoFrame] = rotate_servo_block(block2, { location: 'loop', iteration: 1 }, arduinoFrame);
