...
describe('logic', () => {
  ...
  describe('logic_compare_block', () => {
    it('should return true for EQ if both sides are the same', () => {
      ...
      getInputValueSpy
        .withArgs(block, 'A', true, frameLocation, undefined)
        .and.returnValue(3);
      getInputValueSpy
        .withArgs(block, 'B', false, frameLocation, undefined)
        .and.returnValue(3);
  ...
  describe('control_if_block', () => {
    it('should generate frames that are inside do statement if the if blocks are true', () => {
      getInputValueSpy
        .withArgs(block, 'IF0', true, frameLocation, undefined)
        .and.returnValue(true);

      generateFrameForInputStatementSpy
        .withArgs(block, 'DO0', frameLocation, jasmine.any(ArduinoFrame))
        .and.returnValue([
          ArduinoFrame.makeEmptyFrame('block1', frameLocation, ''),
          ArduinoFrame.makeEmptyFrame('block2', frameLocation, '')
        ]);
  ...
  describe('controls_ifelse_block', () => {
    it('should execute blocks in else if false is connected', () => {
      ...
      generateFrameForInputStatementSpy
        .withArgs(block, 'ELSE', frameLocation, jasmine.any(ArduinoFrame))
        .and.returnValue([
          ArduinoFrame.makeEmptyFrame('block1', frameLocation, ''),
          ArduinoFrame.makeEmptyFrame('block2', frameLocation, '')
        ]);

      const frames = controls_ifelse_block(
        block,
        frameLocation
      ) as ArduinoFrame[];
  ...
  describe('logic_negate_block', () => {
    it('should turn true to false', () => {
      getInputValueSpy
        .withArgs(block, 'BOOL', true, frameLocation, undefined)
        .and.returnValue(false);
