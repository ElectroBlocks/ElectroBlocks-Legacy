  ...
  describe('procedures_callnoreturn_block', () => {
    it('should generate frames for a custom block', () => {
      ...
      const blocklyMock: any | Blockly = {
        mainWorkspace: {
          getTopBlocks: () => {
            return [functionDefinitionBlock, block2];
          }
        }
      };
      ...
      getInputValueSpy
        .withArgs(functionCallBlock, 'ARG0', 0, frameLocation, undefined)
        .and.returnValue(34);

      getInputValueSpy
        .withArgs(functionCallBlock, 'ARG1', '', frameLocation, undefined)
      ...
      spyOn(blockHelper, 'generateFrameForInputStatement')
        ...
        .and.returnValue([
          ArduinoFrame.makeEmptyFrame('block_23', frameLocation, ''),
          ArduinoFrame.makeEmptyFrame('block_23423', frameLocation, '')
        ]);

      const frames = procedures_callnoreturn_block(
        functionCallBlock,
        frameLocation
      );
      ...
      expect(definitionFrameBlock.state.variables['var2'].value).toBe(
        'Hello World'
      );
