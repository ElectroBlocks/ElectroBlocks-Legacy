...
export const led_switch_block = (
    block: Block, frameLocation: FrameLocation, previousFrame?: ArduinoFrame): ArduinoFrame[] => {
    ...
    if (pinState instanceof PinState) {
        ...
        arduinoState.components[index] = new PinState(stringToPin(pin), PIN_TYPE.DIGITAL, state, PinPicture.LED, pinState.color);
    } else {
        const color = randomLedColor();
        arduinoState.components.push(
            new PinState(stringToPin(pin), PIN_TYPE.DIGITAL, state, PinPicture.LED, color));
    }
...
export const led_fade_block = (block: Block, frameLocation: FrameLocation, previousFrame?: ArduinoFrame): ArduinoFrame[] => {

    const arduinoState = previousFrame ? previousFrame.copyState() :
        ArduinoState.makeEmptyState();

    const pin = block.getFieldValue('PIN');

    const fadeNumber = parseInt(getInputValue(
        block, 'FADE', 1, frameLocation, previousFrame
    ).toString());
    ...
    if (pinState instanceof PinState) {
        const index = arduinoState.components.indexOf(pinState);
        arduinoState.components[index] =
            new PinState(stringToPin(pin), PIN_TYPE.ANALOG, fadeNumber, PinPicture.LED, pinState.color);
    } else {
        const color = randomLedColor();
        arduinoState.components.push(
            new PinState(stringToPin(pin), PIN_TYPE.ANALOG, fadeNumber, PinPicture.LED, color));
    }
