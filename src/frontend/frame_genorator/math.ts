 ...
 */
const math_number_block = (block: Block, previousFrame?: ArduinoFrame): number => {
 ...
 */
const math_round_block = (block: Block, previousFrame?: ArduinoFrame) => {

	const op = block.getFieldValue('OP');
	const numberString = getInputValue(block, 'NUM',1, previousFrame).toString();
 ...
 */
const math_modulo_block = (block: Block, previousFrame?: ArduinoFrame) => {

	let dividendValue = getInputValue(block, 'DIVIDEND',  1, previousFrame).toString();

	let dividerValue = getInputValue(block, 'DIVISOR', 1, previousFrame).toString();
 ...
 */
const math_random_int_block = (block: Block, previousFrame?: ArduinoFrame) =>  {

	const fromValue = getInputValue(block, 'FROM',1, previousFrame).toString();

	const toValue = getInputValue(block, 'TO', 10, previousFrame).toString();
