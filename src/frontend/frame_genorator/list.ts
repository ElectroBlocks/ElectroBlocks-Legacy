...
const create_list_number_block_block = (block: Block, previousFrame?: ArduinoFrame) => {
	return createArrayType(block, 'Number List', previousFrame);
};

const create_list_string_block_block = (block: Block, previousFrame?: ArduinoFrame) => {
	return createArrayType(block, 'String List', previousFrame);
};

const create_list_boolean_block_block = (block: Block, previousFrame?: ArduinoFrame) => {
	return createArrayType(block, 'Boolean List', previousFrame);
};

const create_list_colour_block_block = (block: Block, previousFrame?: ArduinoFrame) => {
	return createArrayType(block, 'Colour List', previousFrame);
};


const set_number_list_block_block = (block: Block, previousFrame?: ArduinoFrame) => {
	return setArrayValue(block,  'Number', previousFrame);
};

const set_string_list_block_block = (block: Block, previousFrame?: ArduinoFrame) => {
	return setArrayValue(block,  'String', previousFrame);
};

const set_boolean_list_block_block = (block: Block, previousFrame?: ArduinoFrame) => {
	return setArrayValue(block,  'Boolean', previousFrame);
};

const set_colour_list_block_block = (block: Block, previousFrame?: ArduinoFrame) => {
	return setArrayValue(block,  'Colour', previousFrame);
};

const get_number_from_list_block = (block: Block, previousFrame?: ArduinoFrame) => {
	return getArrayValue(block,  0, 'Number', previousFrame);
};

const get_string_from_list_block = (block: Block, previousFrame?: ArduinoFrame) => {
	return getArrayValue(block,  '', 'String', previousFrame);
};

const get_boolean_from_list_block = (block: Block, previousFrame?: ArduinoFrame) => {
	return getArrayValue(block,  true, 'Boolean', previousFrame);
};

const get_colour_from_list_block = (block: Block, previousFrame?: ArduinoFrame) => {
...
const createArrayType = (block: Block, type: string, previousFrame?: ArduinoFrame) =>  {
	let frame = previousFrame ? previousFrame.makeCopy(block.id) : ArduinoFrame.makeEmptyFrame(block.id);
